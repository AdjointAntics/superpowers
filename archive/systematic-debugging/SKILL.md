...
### Bugs as Equalizer Failure

In category theory, an **equalizer** is the subset where two morphisms agree. When a bug occurs, the implementation diverges from the specification - the equalizer between spec and implementation is broken.

Finding the bug = finding where the implementation fails to equal the specification.
...
The pullback finds exactly which inputs cause the failure - that's the fiber over the error.
...
Fixing root causes = modifying the domain (restores the equalizer).

### The Four Phases in Categorical Terms

| Phase | Categorical Interpretation |
|-------|--------------------------|
| Root Cause Investigation | Finding the equalizer failure |
| Hypothesis Formation | Characterizing the fiber over error |
| Fix Implementation | Restoring the equalizer |
| Verification | Proving spec = implementation |

### Testing as Verification

A passing test after fix proves the equalizer is restored:

```
Test: f(x) = expected
If test passes for all x, then implementation = spec
```
...
- **Backward tracing** (from error): Pull back along the morphism, find the fiber

Both approaches locate the same failure. Use both to confirm.

### Using categorical-reframing

When debugging, invoke `superpowers:categorical-reframing` to:
- Map the bug to equalizer/failure terms
...
|--------------------|----------------------|
| Equalizer | Where spec and implementation agree |
| Fiber over error | The set of inputs causing failures |
| Root cause | Where the morphism fails to preserve structure |
| Symptom | Failure visible in codomain |
| Fix | Restoring the equalizer |
